    ...
    for (int y = 0; y < s->height; y++) {
        for (int x = 0; x < s->width; x += 2) {
            ...
            while (++k < s->iterations) {
                ...
                mk = __builtin_ia32_pfadd(mk, one);
            ...
            mk = __builtin_ia32_pfmul(mk, iter_scale);
            mk = __builtin_ia32_pfmul(__builtin_ia32_pfrsqrt(mk), mk);
            mk = __builtin_ia32_pfmul(mk, depth_scale);

            v2si pixels = __builtin_ia32_pf2id(mk);
